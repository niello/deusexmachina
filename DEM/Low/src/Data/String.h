...
public:
	...
	explicit operator bool() const { return pString && *pString; }
	explicit operator std::string() const { return std::string(pString, Length); }
	...
	bool			operator !=(const CString& Other) const { return Length != Other.Length || (pString != Other.pString && (!pString || !Other.pString || strcmp(pString, Other.pString))); }
	bool			operator <(const CString& Other) const { return operator <(Other.CStr()); }
