...
void CEventClip::PlayInterval(float PrevTime, float CurrTime, Events::IEventOutput& Output, bool IncludeStartTime) const
{
	...
	// NB: PrevTime is exclusive by default because it was fired at the previous PlayInterval call
	...
	{
		// Forward playback
		auto It = IncludeStartTime ?
			std::upper_bound(_Events.cbegin(), _Events.cend(), PrevTime, [](float Time, const CAnimEvent& Clip) { return Time <= Clip.Time; }) :
			std::upper_bound(_Events.cbegin(), _Events.cend(), PrevTime, [](float Time, const CAnimEvent& Clip) { return Time < Clip.Time; });
