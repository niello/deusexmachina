...
protected:
	...
	public:
		...
		virtual void SetScale(U16 Port, const rtm::vector4f& Scale) override { _Blender.SetScale(_Index, Port, Scale); }
		virtual void SetRotation(U16 Port, const rtm::quatf& Rotation) override { _Blender.SetRotation(_Index, Port, Rotation); }
		virtual void SetTranslation(U16 Port, const rtm::vector4f& Translation) override { _Blender.SetTranslation(_Index, Port, Translation); }
		virtual void SetTransform(U16 Port, const rtm::qvvf& Tfm) override { _Blender.SetTransform(_Index, Port, Tfm); }
	};

	std::vector<CInput>    _Sources;
	std::vector<U8>        _SourcesByPriority;
	std::vector<rtm::qvvf> _Transforms;   // per port per source
...
public:
	...
	void SetScale(U8 Source, U16 Port, const rtm::vector4f& Scale)
	{
	...
	void SetRotation(U8 Source, U16 Port, const rtm::quatf& Rotation)
	{
		const UPTR Idx = Port * _Sources.size() + Source;
	...
	void SetTranslation(U8 Source, U16 Port, const rtm::vector4f& Translation)
	{
		const UPTR Idx = Port * _Sources.size() + Source;
	...
	void SetTransform(U8 Source, U16 Port, const rtm::qvvf& Tfm)
	{
		const UPTR Idx = Port * _Sources.size() + Source;
		_Transforms[Idx] = Tfm;
