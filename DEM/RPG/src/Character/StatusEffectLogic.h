...
void TriggerStatusEffects(Game::CGameSession& Session, const Game::CGameWorld& World, Game::HEntity EntityID, CStrID Event, Game::CGameVarStorage& Vars, TPolicy Policy = {})
{
	const auto* pStatusEffectComponent = World.FindComponent<const CStatusEffectsComponent>(EntityID);
	if (pStatusEffectComponent) return;
