...
void CChoiceAction::CollectChoicesFromLink(CChoiceAction& Root, const Flow::CFlowLink& Link, Game::CGameSession& Session, bool DebugMode, bool IsValid)
{
	...
	if (CPhraseAction::RTTI.IsBaseOf(pLinkedRTTI))
	{
		...
		const auto Speaker = Game::ResolveEntityID(pActionData->Params, sidSpeaker, &Root._pPlayer->GetVars());
		...
		Text.append(static_cast<std::string_view>(pActionData->Params->Get<std::string>(sidText, EmptyString)));
