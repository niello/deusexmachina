...
public:
	...
	const char*		GetRootPath() const { return RootPath.IsEmpty() ? NULL : RootPath.Get(); }
};
...
inline CNpkTOCEntry* CNpkTOC::FindEntry(const char* pAbsPath)
{
	...
	if (strncmp(RootPath.Get(), pAbsPath, RootPath.Length())) return NULL;
	...
	char* pCurrToken = strtok((char*)LocalPath.Get(), "/\\");
