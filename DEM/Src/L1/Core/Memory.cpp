...
void* n_malloc_dbg(size_t size, const char* filename, int line)
{
	...
	if (DEM_LogMemory) Core::Log("%lx = n_malloc(size=%d, file=%s, line=%d)\n", res, size, filename, line);
	return res;
}
//---------------------------------------------------------------------
...
void* n_calloc_dbg(size_t num, size_t size, const char* filename, int line)
{
	...
	if (DEM_LogMemory) Core::Log("%lx = n_calloc(num=%d, size=%d, file=%s, line=%d)\n", res, num, size, filename, line);
	return res;
}
//---------------------------------------------------------------------
...
void* n_realloc_dbg(void* memblock, size_t size, const char* filename, int line)
{
	...
	if (DEM_LogMemory) Core::Log("%lx = n_realloc(ptr=%lx, size=%d, file=%s, line=%d)\n", res, memblock, size, filename, line);
	return res;
}
//---------------------------------------------------------------------
...
void n_free_dbg(void* memblock, const char* filename, int line)
{
	...
	if (DEM_LogMemory) Core::Log("n_free(ptr=%lx, file=%s, line=%d)\n", memblock, filename, line);
}
//---------------------------------------------------------------------
...
void* operator new(size_t size)
{
	...
	if (DEM_LogMemory) Core::Log("%lx = new(size=%d)\n", res, size);
	return res;
}
//---------------------------------------------------------------------
...
void* operator new(size_t size, const char* file, int line)
{
	...
	if (DEM_LogMemory) Core::Log("%lx = new(size=%d, file=%s, line=%d)\n", res, size, file, line);
	return res;
}
//---------------------------------------------------------------------
...
{
	void *res = place;
	if (DEM_LogMemory) Core::Log("%lx = new(size=%d, place=0x%x, file=%s, line=%d)\n", res, size, place, file, line);
	return res;
}
//---------------------------------------------------------------------
...
{
	if (DEM_LogMemory) Core::Log("delete(ptr=%lx)\n", p);
