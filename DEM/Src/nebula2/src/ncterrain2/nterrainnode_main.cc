...
*/
bool nTerrainNode::LoadResources()
{
    if (!nMaterialNode::LoadResources()) return false;
...
nTerrainNode::RenderGeometry(nSceneServer* sceneServer, nRenderContext* renderContext)
{
    ...
    // resource may not be available yet if async resource loading enabled
	if (!this->refChunkLodTree->IsValid()) return false;
