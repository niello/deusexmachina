	...
	bool ExportModel(const std::string& MeshName, CContext& Ctx, Data::CDataArray& Attributes)
	{
		...
		if (!Mesh.extras.empty())
		{
			...
			if (!ShapeType.empty())
			{
				...
				Ctx.Log.LogInfo(std::string("Mesh ") + Mesh.name + " has an autogenerated collision shape: " + ShapeType);

				// FIXME: calculate this node's final scaling, must save into a shape asset to avoid unnecessary unsharing!
				// Don't save if 1, 1, 1
				...
				{
					//???warning?
					Ctx.Log.LogError(std::string("Mesh ") + Mesh.name + ": can't generate collision shape");
					return false;
				}
				...
				const auto ShapePath = Ctx.CollisionPath / (MeshRsrcName + ".hrd");
				if (!ParamsUtils::SaveParamsToHRD(ShapePath.string().c_str(), CollisionShape))
				{
					Ctx.Log.LogError("Error saving colision shape to " + ShapePath.generic_string());
					return false;
				}
