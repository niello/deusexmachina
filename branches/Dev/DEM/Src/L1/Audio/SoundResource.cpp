...
bool CSoundResource::LoadResource()
{
	...
	else Sys::Error("Audio file format not supported: %s\n", FileName.CStr());
	n_assert(pWaveFile);
	...
	if (Streaming) // create a streaming sound object (with a 128 KByte streaming buffer)
	{
		...
		if (FAILED(pDS->CreateSoundBuffer(&DSBufDesc, &pDSBuffer, NULL)))
			Sys::Error("CSoundResource::LoadResource(): CreateSoundBuffer for '%s' failed!", FileName.CStr());
	...
	else
	{
		...
		if (FAILED(pDS->CreateSoundBuffer(&DSBufDesc, &pDSBuffers[0], NULL)))
		{
			...
			Sys::Error("CSoundResource::LoadResource(): CreateSoundBuffer for '%s' failed!"
