...
{
	DWORD FileAttrs = GetFileAttributes(DataSrv->ManglePath(Path).Get());
...
bool CFileSystemWin32::SetFileReadOnly(const nString& Path, bool ReadOnly)
{
	...
	DWORD FileAttrs = GetFileAttributes(AbsPath.Get());
	...
	return SetFileAttributes(AbsPath.Get(), FileAttrs) != FALSE;
}
//---------------------------------------------------------------------
...
#endif
	return ::DeleteFile(DataSrv->ManglePath(Path).Get()) != 0;
}
//---------------------------------------------------------------------
...
{
	return RemoveDirectory(DataSrv->ManglePath(Path).Get()) != FALSE;
}
//---------------------------------------------------------------------
...
void* CFileSystemWin32::OpenDirectory(const nString& Path, const nString& Filter, nString& OutName, EFSEntryType& OutType)
{
	...
	char* pFilter = Filter.IsValid() ? Filter.Get() : "/*.*"; //???or "/*" ?

	WIN32_FIND_DATA FindData;
	HANDLE hDir = FindFirstFile((AbsPath + pFilter).Get(), &FindData);
...
void* CFileSystemWin32::OpenFile(const nString& Path, EStreamAccessMode Mode, EStreamAccessPattern Pattern)
{
	...
	nString AbsPath = DataSrv->ManglePath(Path);
	HANDLE hFile = CreateFile(	AbsPath.Get(),
								Access,
								ShareMode,
								0,
								Disposition,
...
bool CFileSystemWin32::Seek(void* hFile, int Offset, ESeekOrigin Origin)
{
	...
	switch (Origin)
	{
		case SSO_CURRENT:	SeekOrigin = FILE_CURRENT; break;
		case SSO_END:		SeekOrigin = FILE_END; break;
