...
DB::PDatabase OpenDB(const nString& SrcDBPath)
{
	...
	pDB.Create();
	pDB->URI = SrcDBPath;
	pDB->SetExclusiveMode(true);
	pDB->AccessMode = DB::CDatabase::DBAM_ReadOnly;
	pDB->SetIgnoreUnknownColumns(true);
	pDB->SetInMemoryDatabase(false);
	if (!pDB->Open())
	{
		nString Path = DataSrv->ManglePath(SrcDBPath);
		n_error("Failed to open database '%s'!\nError: %s", Path.Get(), pDB->GetError().Get());
		return NULL;
	}
	return pDB;
}
...
int FailApp(bool WaitKey)
{
	n_printf("Building aborted due to errors.\n");
	if (WaitKey)
	{
...
int main(int argc, const char** argv)
{
	...
	ProjDir = args.GetStringArg("-proj");
	nString StaticDBFile = args.GetStringArg("-staticdb");
	nString GameDBFile = args.GetStringArg("-gamedb");
	nString BuildDir = args.GetStringArg("-build");
	bool WaitKey = !args.GetBoolArg("-nowait");
	//!!!-v!
