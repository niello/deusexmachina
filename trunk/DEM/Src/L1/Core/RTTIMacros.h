...
public: \
    ...
    static Core::CRefCounted* FactoryCreator(); \
    static type* Create(); \
...
#define __ImplementClass(type, fourcc, baseType) \
    ...
    Core::CRefCounted* type::FactoryCreator() { return type::Create(); } \
    type* type::Create() \
    { \
        CRefCounted::criticalSection.Enter(); \
        CRefCounted::isInCreate = true; \
        type* newObject = n_new(type); \
        CRefCounted::isInCreate = false; \
        CRefCounted::criticalSection.Leave(); \
        return newObject; \
    }\
