...
bool CFileSystemNPK::Mount(const nString& Source, const nString& Root)
{
	...
	nString RealRoot = DataSrv->ManglePath(Root);

	TOC.SetRootPath(RealRoot.Get());
	...
	while (InsideTOC)
	{
		...
		if (FourCC == 'DIR_')
		{
			...
			if (!strcmp(NameBuffer, "<noname>"))
			{
				...
				NewName.ToLower();
				n_strncpy2(NameBuffer, NewName.Get(), sizeof(NameBuffer));
			}
...
{
	CNpkTOCEntry* pTE = TOC.FindEntry(DataSrv->ManglePath(Path).Get());
...
DWORD CFileSystemNPK::Read(void* hFile, void* pData, DWORD Size)
{
	...
	NPKData.Seek(pTE->GetFileOffset() + Pos, SSO_BEGIN);
...
bool CFileSystemNPK::Seek(void* hFile, int Offset, ESeekOrigin Origin)
{
	...
	switch (Origin)
	{
		case SSO_CURRENT:	SeekPos = ((CNPKFile*)hFile)->Offset + Offset; break;
		case SSO_END:		SeekPos = Len + Offset; break;
