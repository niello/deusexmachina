...
BEGIN_ATTRS_REGISTRATION(QuestSystem)
...
namespace Story
{
ImplementRTTI(Story::CQuestSystem, Core::CRefCounted);
ImplementFactory(Story::CQuestSystem);

using namespace Data;

CQuestSystem* CQuestSystem::Singleton = NULL;

CQuestSystem::CQuestSystem()
{
    n_assert(!Singleton);
    Singleton = this;
	DataSrv->SetAssign("quests", "game:quests");
	SUBSCRIBE_PEVENT(OnLoad, CQuestSystem, OnLoad);
	SUBSCRIBE_PEVENT(OnSave, CQuestSystem, OnSave);
}
//---------------------------------------------------------------------
...
void CQuestSystem::Trigger()
{
	if (QuestsToDelete.Size()) QuestsToDelete.Clear();
...
bool CQuestSystem::LoadQuest(CStrID QuestID, CStrID* OutStartingTaskID)
{
	...
	if (OutStartingTaskID)
		*OutStartingTaskID = CStrID(QuestDesc->Get<nString>(CStrID("StartingTask"), "").Get());
...
bool CQuestSystem::StartQuest(CStrID QuestID, CStrID TaskID)
{
	if (QuestID == CStrID::Empty || GetQuestStatus(QuestID, TaskID) != CQuest::No) FAIL;

	n_printf("QuestSys: starting quest %s, task %s\n", QuestID.CStr(), TaskID.CStr());
...
#ifdef _DEBUG
	n_printf("TASK \"%s\" started. %s\n", Task.Task->Name.Get(), Task.Task->Description.Get());
#endif
	...
	if (DataSrv->FileExists(TaskScriptFile)) //???is optimal?
	{
		nString Name = nString(QuestID.CStr()) + StrUnderline + TaskID.CStr();
		Name.ReplaceChars("/", '_');
		Task.Task->ScriptObj = n_new(Scripting::CScriptObject(Name.Get(), StrQuests.Get()));
...
//!!!CODE DUPLICATIONS!
bool CQuestSystem::CloseQuest(CStrID QuestID, CStrID TaskID, bool Success)
{
	int Idx = Quests.FindIndex(QuestID);
...
CQuest::EStatus CQuestSystem::GetQuestStatus(CStrID QuestID, CStrID TaskID)
{
	int Idx = Quests.FindIndex(QuestID);
...
bool CQuestSystem::OnLoad(const Events::CEventBase& Event)
{
		...
		if (TaskID.IsValid())
		{
			...
			if (TaskRec.Status == CQuest::Opened)
			{
				...
				if (DataSrv->FileExists(TaskScriptFile)) //???is optimal?
				{
					nString Name = nString(QuestID.CStr()) + StrUnderline + TaskID.CStr();
					Name.ReplaceChars("/", '_');
					TaskRec.Task->ScriptObj = n_new(Scripting::CScriptObject(Name.Get(), StrQuests.Get()));
